...
function twentyseventeen_custom_header_setup() {
     ...
     * }
     */
    $default_uri = get_parent_theme_file_uri('/assets/images/header.jpg');
    if (defined(FLICKR_API_KEY) && !empty(FLICKR_API_KEY)) {
        $photoset_url = "https://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=" . FLICKR_API_KEY . "&photoset_id=" . FLICKR_PHOTOSET_ID . "&user_id=" . FLICKR_USER_ID . "&format=json&nojsoncallback=1";
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $photoset_url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $output = curl_exec($ch);
        curl_close($ch);
        
        $photoset = json_decode($output);
        $photoset = $photoset->photoset->photo;
        $photo_id = FALSE;
        if ( count($photoset) == 1) {
            $photo_id = $photoset[0]->id;
        } else if ( count($photoset) > 1 ) {
            $index = rand(0, count($photoset));
            $photo_id = $photoset[$index]->id;
        }
        if ( $photo_id !== FALSE ) {
            $photo_url = "URL: https://api.flickr.com/services/rest/?method=flickr.photos.getSizes&api_key=" . FLICKR_API_KEY . "&photo_id=" . $photo_id . "&format=json&nojsoncallback=1";
            $ch = curl_init();
            ...
            $sizes = $result->size;
            foreach ( $sizes as $photo ) {
                if ( $photo->width > 2000) {
                    $default_uri = $photo->source;
                    break;
                }
            }
        }
